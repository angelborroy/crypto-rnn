...
# Py3: needed for reduce + (some code uses itertools.accumulate via components; harmless to import here)
...
class EnigmaConfig(object):
    ...
    def config_enigma(rotor_names, window_letters, plugs, rings):
        ...
        if not (len(ring_numbers) == len(window_letters) == len(rotor_names_list)-1):
            raise EnigmaValueError(
                'Bad configuration: number rotors ({0}), rings ({1}), and window letters ({2}) must match'.format(
    ...
    def config_enigma_from_string(string):
        ...
        parts = [s for s in string.split(' ') if s != '']  # Py3: filter to list
        if len(parts) != 4:
            raise EnigmaValueError('Bad string - {0} should have 4 elements'.format(parts))
        rotor_names, window_letters, plugs, rings = parts
    ...
    # Py3: __str__ must return str (not bytes); avoid calling builtin 'unicode'
    def __str__(self):
        return self.__unicode__()

    def __repr__(self):
        return '{0} ({1})'.format(object.__repr__(self), self.__unicode__())

    def __eq__(self, cfg):
        return all([self.components == cfg.components, self.positions == cfg.positions,  self.rings == cfg.rings])

    def __hash__(self):
        # Hash only immutable state that defines equality
        return hash((self._components, self._positions, self._rings))
    ...
    def _make_enigma_char(letter):
        # Py3: filter returns iterator; get first element safely
        return next((l for l in (letter + ' ').upper() if l in LETTERS + ' '))

    def _config_string(self, letter, mark_func=None):
        cfg_mapping = self.enigma_mapping()
        return '{0} {1}  {2}  {3}'.format(
            letter + ' >' if letter in LETTERS else '   ',
            EnigmaConfig._marked_mapping(
                cfg_mapping,
                EnigmaConfig._locate_letter(cfg_mapping, letter, cfg_mapping),
                mark_func
            ),
            self.windows(),
    ...
    def make_message(string):
        ...
        # Py3: lambda s,(o,n) -> lambda s,on  and make filter produce a string
        msg = "".join(
            filter(
                lambda c: c in LETTERS,
                reduce(lambda s, on: s.replace(on[0], on[1]), subs, string.upper())
            )
        )
    ...
    def config_string(self, letter='', format='single', show_encoding=False, mark_func=None):
        ...
        # !!! original note about UnicodeDecodeError does not apply in Py3 once __str__ returns text
